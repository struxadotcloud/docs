...
<Callout type="warn">
  In production, run Wings on separate machines from the Panel. Keeping the Panel, MySQL, and game server containers on a single host can cause resource contention under load.
</Callout>
