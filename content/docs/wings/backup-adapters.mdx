...
**Delete** — best-effort `DELETE files/{id}`. Failures (e.g. revoked access) are swallowed so the Panel row can still be removed; the Drive file is then left for the user to clean up.
...
<Callout type="warn">
  The config response for a Google Drive backup contains the user's OAuth tokens and the operator's OAuth client secret — the same trust model as the Proxmox Backup Server adapter, which already receives `token_secret` from the Panel. A Wings node (or its node token) can therefore read backups of its servers' users; protect node tokens accordingly.
</Callout>
