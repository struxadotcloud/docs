...
- **Webserver** - use your existing nginx, Apache, or Caddy, install nginx fresh, or skip (Docker Compose only)
...
5. **Configures a webserver** - writes the reverse proxy config for the Panel.
...
| `/opt/wings/config/config.yml` | Wings configuration (Dashboard + Wings mode) |
