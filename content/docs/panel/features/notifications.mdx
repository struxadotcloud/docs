...
- **Telegram** — a bot token and chat ID. Create a bot with [@BotFather](https://t.me/BotFather) and grab its token. To find a chat ID, send the bot a message, then open `https://api.telegram.org/bot<token>/getUpdates` in your browser — the chat ID is in the response. Groups and channels work the same way; add the bot to them first.
