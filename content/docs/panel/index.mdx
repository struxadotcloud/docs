...
- **Backups** - Local, S3, restic, PBS, kopia, and per-user Google Drive backups with operator-configurable destinations (see [Backups](/panel/features/backups))
- **Notifications** - Discord and Telegram alerts for server and panel events, with admin and per-user endpoints (see [Notifications](/panel/features/notifications))
...
<Callout type="warn">
  For production, run Wings on separate machines from the Panel so game servers and the Panel don't compete for the same resources.
</Callout>
